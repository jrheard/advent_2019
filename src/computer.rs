...
mod tests {
    ...
    fn test_larger_example_program_from_5b() {
        ...
        assert_eq!(run_program(large_program.clone(), vec![5]).1, vec![999]);
        assert_eq!(run_program(large_program.clone(), vec![8]).1, vec![1000]);
        assert_eq!(run_program(large_program, vec![12]).1, vec![1001]);
    }
}
